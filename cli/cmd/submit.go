...
import (
	...
	"github.com/benekuehn/so/internal/gh"
...
- Stores PR numbers locally in '.git/config' for future updates.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// 2. Get Stack Info
		_, stack, _, err := getCurrentStackInfo() // Use helper from restack
		if err != nil {
		...
		for i := 1; i < len(stack); i++ {
			...
			if prNumber > 0 {
				...
				if err != nil {
					...
					if existingPR.GetBase().GetRef() != parent {
						...
						_, errUpdate := ghClient.UpdatePullRequestBase(prNumber, parent)
						if errUpdate != nil {
					...
					// TODO: Add logic to update draft status, title, body based on flags later?
					fmt.Printf("PR #%d link: %s\n", prNumber, existingPR.GetHTMLURL())
					continue // Move to next branch in stack
				}

			}
			// If we reach here, we need to create a new PR (either no stored #, or fetch failed)
			// --- Create New PR ---
			fmt.Printf("Creating new PR for branch '%s' -> '%s'...\n", branch, parent)
			...
			if errTpl != nil {
				// Log error reading template but proceed without it
			...
			editorPrompt := &survey.Editor{
				...
				FileName:      "*.md",
				Default:       templateContent,       // Use template if found
				HideDefault:   templateContent == "", // Don't show default if it's empty
				AppendDefault: true,                  // Append template to user input? No, let them edit it.
			}
		...
		fmt.Println("\nSubmit process complete for the current stack.")
		return nil
	},
}
...
func init() {
	// AddCommand(submitCmd) // Called in root.go
