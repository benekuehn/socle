...
func baseMultiStackSelection(
    ...
    for i, opt := range options {
        if opt == selectedOption { selectedIndex = i; break }
    }
    if selectedIndex == -1 { return "", true, fmt.Errorf("internal error: could not find selected option") }
...
func navigateLinear(
    ...
    if stack == nil || len(stack) == 0 { return "", true, fmt.Errorf("internal error: empty stack for branch '%s'", currentBranch) }
    idx := cmdutils.FindIndexInStack(currentBranch, stack)
    if idx == -1 { return "", true, fmt.Errorf("internal error: branch '%s' not found in stack %v", currentBranch, stack) }
    switch purpose {
    case "up":
        if idx == len(stack)-1 { _, _ = fmt.Fprintf(stdout, "Already on the top branch: '%s'.\n", currentBranch); return "", true, nil }
        return stack[idx+1], true, nil
    case "top":
        if idx == len(stack)-1 { _, _ = fmt.Fprintf(stdout, "Already on the top branch: '%s'\n", currentBranch); return "", true, nil }
        return stack[len(stack)-1], true, nil
    case "bottom":
        if len(stack) <= 1 { _, _ = fmt.Fprintf(stdout, "Already on the base branch '%s', which is the only branch in the stack.\n", currentBranch); return "", true, nil }
        if idx == 1 { _, _ = fmt.Fprintf(stdout, "Already on the bottom branch: '%s'\n", currentBranch); return "", true, nil }
